...
from utils.tools import set_seed, read_yaml, save_results, AttributeDict
...
parser.add_argument('--framework_name', type=str, default='rationale')
...
RUN_FRAMEWORK_NAME = 'FACTUAL'
PROJECT_NAME = 'Mitigating Bias on Stance Detection'
...
from utils.dataset_utils.data_config import factual_datasets as datasets
...
# if to save the model states and test results
SAVE_STATE = False
...
if not os.path.isdir(OUTPUT_DIR):
    os.mkdir(OUTPUT_DIR)
CUR_DIR = OUTPUT_DIR + f'/{DATASET_NAME}_{GLOBAL_ARGS.framework_name}_{GLOBAL_ARGS.llm}_{GLOBAL_ARGS.model_name}_' + datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
...
# load config
CONFIG_PATH = {
    'rationale': 'FACTUAL/configs/rationale_config.yaml',
    'cad': 'FACTUAL/configs/cad_config.yaml'
}
...
from models import TransformerModel as Model
...
def evaluate(eval_data_loader, model, criterion):
    ...
    return (eval_data_loader.dataset.raw_sentences, eval_data_loader.dataset.raw_targets, true_labels.tolist(), predict_labels.tolist()), loss_mean / len(eval_data_loader), acc, f1, precision, recall
...
def train_process(args, train_data_loader, valid_data_loader, test_data_loader, fold=0, target_name=None):
    ...
    for ep in range(args.num_epochs):
        ...
        if test_f1 > best_all_test_f1:
            best_all_test_f1 = test_f1
    ...
    return best_results, best_test_acc, best_all_test_f1, best_test_precision, best_test_recall

def main(args=None):
    if not GLOBAL_ARGS.debug_mode and not GLOBAL_ARGS.normal:
        run_name = f'{RUN_FRAMEWORK_NAME}_{DATASET_NAME}_{GLOBAL_ARGS.framework_name}_{GLOBAL_ARGS.llm}_{GLOBAL_ARGS.model_name}_{datetime.now().strftime("%Y-%m-%d_%H:%M:%S")}'
    ...
    final_score = 0
    targeted_result = {}
    target_names = data_config.in_target_target_names if not GLOBAL_ARGS.zero_shot else data_config.zero_shot_target_names
    ...
    for fold in range(args.train_times):
        ...
        overall_pred = []
        for target_name in target_names:
            tweet_texts, targets, true_labels, predict_labels = targeted_result[data_config.short_target_names[target_name]][fold]
